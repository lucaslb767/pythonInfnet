...
def ball_collision(ball, bar1, bar2, ballDirX):
    if ballDirX < 0 and  bar1.right == ball.left and bar1.top <= ball.top and bar1.bottom >= ball.bottom:
        return -1
...
def score_check(bar1, ball, score, ballDirX):
    ...
    if ball.left == LINE_WIDTH:
        ...
        score += 10
        return score
    elif ballDirX > 0 and bar1.right == ball.left and bar1.top < ball.top and bar1.bottom > ball.bottom:
        score += 1
        return score
